import ldap  # pip install python-ldap (sudo apt-get install python-dev)
...
def get_config(peer_private_key, peer_ip):
    return f'''[Interface]
Address = {peer_ip}
...
#   Генерирует конфиг файл и QR код (если в это надо, включается в конфиг файле)
def gen_peer_config(peer_private_key, peer_ip, peer_name):
    peer_ip = str(peer_ip) + '/' + str(config.peer_ip_mask)
    config_peer = get_config(peer_private_key, peer_ip)
...
def send_email(peer_pub_key):
    ...
    html = open(config.smtp_template_html, "rb").read().decode()

    ###########################################
    html = html.replace('{href_android}', config.href_android)
    html = html.replace('{href_macOS}', config.href_macOS)
    html = html.replace('{href_windows}', config.href_windows)
    html = html.replace('{href_ios}', config.href_ios)
    html = html.replace('{how_to_src}', config.how_to_src)
    ...
    config_peer = get_config(peer_private_key, peer_ip)  # Генерируем текст конфига
